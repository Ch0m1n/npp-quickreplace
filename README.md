...
- 여러 줄 치환과 `${cursor}`, `${tabstop:1}`→`${tabstop:2}`→`${tabstop:0}` Tab 순회 세션
- `${date}`, `${time}`, `${filename}`, `${filepath}`, `${clipboard}`, `${selection}`, `${uuid}`, `${line}`, `${column}` 내장 변수
- 한 번의 Notepad++ Undo 작업으로 되돌리는 RAII 편집 트랜잭션
- 최대 256개 다중 커서를 사전 검증한 뒤 한 번의 Undo 작업으로 적용하는 일괄 치환
- Windows 표시 언어를 자동 감지하는 한국어·영어 메뉴, 규칙 관리자, 그룹 관리자 UI
- 자동·English·한국어를 직접 선택할 수 있는 DPI 대응 설정 창과 원자적 설정 저장
- 현재 초안을 실제 조건으로 검사하고 필터 차단 원인을 알려주는 규칙 테스트 창
- 외부 `replacements.json` 변경 자동 감지와 잘못된 파일에 대한 마지막 정상 규칙 유지
- 설정과 규칙을 하나로 옮기는 검증된 데이터 패키지, 가져오기 전 복구 복사본
...
- 일반 정규식은 UI 스레드 정지 위험 때문에 받지 않아요. 대신 `matchMode: "captureTemplate"`의 `${capture:1}`~`${capture:9}`를 지원해요. 캡처 사이는 리터럴 구분자가 필요하고, 인접 캡처와 즉시 치환은 허용하지 않아요.
- 다중 커서 치환에서는 각 커서를 한 번의 Undo로 처리하지만, 탭스톱 순회 세션은 단일 커서 치환에서만 유지해요. 다중 커서는 첫 탭스톱 위치에 각각 놓여요.
...
릴리스 ZIP은 DLL이 루트에 있는 Plugin Admin 호환 구조예요. `plugins\NppQuickReplace` 폴더를 만든 뒤 그 안에 풀면 돼요.
...
# ARM64용 Visual Studio C++ 도구가 설치되어 있다면
...
build/NppQuickReplace-0.7.0-x64.zip
```
...
# 가장 최근 설치 백업으로 되돌리기
pwsh -NoLogo -NoProfile -File .\scripts\install-local.ps1 -Action Rollback

# DLL을 삭제하지 않고 비활성 격리하기
pwsh -NoLogo -NoProfile -File .\scripts\install-local.ps1 -Action Uninstall
```
...
플러그인 메뉴:
메뉴와 관리 창은 Windows 표시 언어가 한국어면 한국어로 표시되고, 그 외 언어에서는 영어로 표시돼요.
...
- `Settings…`: 언어, 자동 재로드, 입력 동작, 제한, 백업, 로그 및 데이터 패키지 관리

규칙 관리자에서는 좌측 목록을 검색·필터·정렬하고, 우측에서 트리거·캡처 템플릿 모드·치환문·그룹·실행 조건·확장자·경로 글롭·언어·설명을 편집하고 결과를 미리 볼 수 있어요. 캡처 템플릿을 켜면 충돌하는 Immediate 옵션은 자동으로 꺼져요. 좌측 다중 선택 뒤 `Set state…`로 한꺼번에 켜거나 끌 수 있어요. `Groups…`에서는 그룹 추가, ID·이름 변경, 활성화, 삭제를 다루며 그룹 ID가 바뀌면 연결된 규칙도 함께 갱신돼요.
...
캡처 템플릿은 정규식 대신 리터럴 구분자 사이를 최대 9개까지 안전하게 추출해요.
...
인접한 캡처 둘은 경계가 모호해서 거부하고, 치환문이 트리거에 없는 캡처 번호를 참조해도 저장하지 않아요. 동일 입력에 여러 캡처 템플릿이 맞으면 규칙 파일에서 먼저 나온 항목을 사용해요.
...
여러 줄 템플릿에서 `${cursor}` 위치에 커서가 놓여요. `${tabstop:1}`~`${tabstop:3}` 등 1~9는 번호 순서로 이동하고 `${tabstop:0}`은 마지막 위치예요. 0이 없고 `${cursor}`가 함께 있으면 커서 표식이 마지막 정지점이 돼요. Tab을 누르면 다음 위치로 이동하고 마지막 위치에서는 일반 Tab 입력으로 돌아가요.
...
- IME 임시 조합 문자는 무시해요.
- 읽기 전용 문서는 건너뛰어요. 다중 커서는 기본 지원하며 `skipMultiSelection`을 `true`로 바꾸면 예전처럼 자동·수동 치환을 건너뛸 수 있어요.
- 다중 커서는 모든 위치의 규칙이 일치하고 범위가 겹치지 않을 때만 전체 적용하며, 최대 256개와 합산 출력 크기 한도를 둬요.
...
src/CapturePattern.*  선형 시간 캡처 템플릿 컴파일·매칭
src/SnippetTemplate.* 탭스톱·커서 표식 파싱
src/ConfigStore.*     설정 보존, 원자 저장, 자동 백업
src/DataPackage.*     설정·규칙 통합 패키지와 복구 복사본
src/RuleExchange.*    CSV/TSV 파싱, 검증, 내보내기
src/RuleManager.*     Win32 규칙 관리자 GUI
src/RuleTester.*      현재 초안 일치·차단 진단 GUI
src/SettingsManager.* 설정·언어·데이터 관리 GUI
...
- 0.8: 다중 커서 탭스톱 동시 순회, 후보 팝업, 사용 통계 검토
- 1.0: 안정화된 호스트 호환성 매트릭스, 코드 서명, Plugin Admin 등록
