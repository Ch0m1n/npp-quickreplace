...
- Version target: `0.6.0-alpha` for preview testing.
...
- Submit to Plugins Admin only after promoting the build to a non-alpha `0.6.0.0` binary and publishing an immutable stable release URL.
...
- `build/NppQuickReplace-0.6.0-x64.zip` exists.
...
2. Keep the numeric binary version and Plugin Admin version exactly aligned, for example `0.6.0.0`.
3. Build from a clean tagged commit.
4. Sign the DLL when a trusted certificate is available, then package it.
5. Upload the ZIP and `SHA256SUMS.txt` to an immutable `v0.6.0` GitHub release.
...
pwsh -NoLogo -NoProfile -File .\scripts\generate-plugin-admin-entry.ps1 `
  ...
  -Repository https://github.com/Ch0m1n/npp-quickreplace/releases/download/v0.6.0/NppQuickReplace-0.6.0-x64.zip
```
